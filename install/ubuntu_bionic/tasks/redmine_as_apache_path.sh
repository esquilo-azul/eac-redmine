...
TEMPLATE="$INSTALL_ROOT2/template/redmine_as_apache_path.conf"
...
function task_condition {
  ...
  return $("$INSTALL_ROOT2/lib/text/template.sh" "$TEMPLATE" | "$INSTALL_ROOT2/lib/text/diff-stdin-file.sh" "$AVAILABLE_PATH" )
}
export -f task_condition

function task_execute {
  "$INSTALL_ROOT2/lib/text/template.sh" "$TEMPLATE" | sudo tee "$AVAILABLE_PATH" > /dev/null
