    ...
    module Files
      ...
      def fixtures_directory
        @fixtures_directory ||= begin
          directory = fixtures_directory_path
      ...
      def fixtures_directory_path
        test_path = Pathname.new(File.expand_path(class_file_path))
        test_path.ascend do |p|
          if p.parent && p.parent.basename.to_s == 'test'
            return File.expand_path("fixtures/#{test_path.relative_path_from(p)}".chomp('.rb'),
                                    p.parent)
          end
        end
        fail 'Fixtures directory path not found'
      end
    end
  end
end
