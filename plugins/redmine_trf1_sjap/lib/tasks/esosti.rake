namespace :trf1_sjap do
  namespace :esosti do
    ...
    task :reset_solicitacao_import, [:esosti_id] => [:environment] do |_t, args|
    ...
    task :import_solicitacao_detalhes, [:project_identifier, :esosti_id] => [:environment] do |_t, args|
    ...
    task :replicate_solicitacao, [:esosti_id] => [:environment] do |_t, args|
      esosti_solicitacao = EsostiSolicitacao.find_by_esosti_id(args.esosti_id)
      unless esosti_solicitacao
        puts "Solicitação e-Sosti não encontrada com esosti_id=#{esosti_id}"
        break
