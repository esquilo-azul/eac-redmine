  ...
  class EsostiRedmineImport
    class SolicitacaoDetalhesReplicate
      ...
      def initialize(esosti_solicitacao, propriedades, updates)
        ...
        fail 'propriedades.empty?' if propriedades.empty?
        fail "updates[0][:fase] != FASE_CADASTRO_DESCRICAO (#{esosti_solicitacao.esosti_id}, \"#{updates[0][:fase]}\")" if updates[0][:fase] != SolicitacaoDetalhes::FASE_CADASTRO_DESCRICAO
