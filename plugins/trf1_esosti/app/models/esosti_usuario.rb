...
class EsostiUsuario < ActiveRecord::Base
  ...
  def self.parse_esosti_usuario_rotulo(esosti_usuario_rotulo)
    ...
    fail "Rótulo de usuário e-Sosti não coincide com o formato esperado: \"#{esosti_usuario_rotulo}\" (Formato esperado: <MATRÍCULA> - <NOME>)" unless parts
  ...
  def self.capitalize_name(names, limit)
    ...
    while result.join(' ').strip.length > limit
      ...
      if new_result == result
        fail "Nome não pôde ser abreviado: #{result.join(' ').strip}"
      end
      result = new_result
      x = result.join(' ').strip
    end
  ...
  def self.abreviate_name(parts)
    ...
    result = abreviate_middle(parts, false) if result == parts
    result
  end
